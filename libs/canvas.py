...
class Canvas(QWidget):
    ...
    samResultReady = pyqtSignal(object)  # SAM 推理完成，dict {type, mask, bbox}
    samEncodeRequested = pyqtSignal()
    shapeDoubleClicked = pyqtSignal()    # SAM 需要重新编码当前图片
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sam_mode = "point"       # "point" | "box"
        self.sam_drag_start = None    # 框提示拖拽起点
        self.sam_drag_rect = None     # 拖拽中的虚线矩形
        self.sam_passthrough = False   # 按住 Tab 时穿透形状触发 SAM（大框内小目标）
        self.drawing_mode = "rect"  # "rect" or "polygon"
        self.draw_by_two_clicks = False
    ...
    def set_drawing_mode(self, mode):
        """Set drawing mode to "rect" or "polygon"."""
        self.drawing_mode = mode

    def enterEvent(self, ev):
        if self.sam_passthrough:
            return
    ...
    def mouseMoveEvent(self, ev):
        ...
        # --- SAM 模式 ---
        if self.sam_enabled and not self.drawing():
            if self.sam_mode == "box" and self.sam_drag_start:
                # 拖拽中：记录虚线框
                self.sam_drag_rect = QRectF(self.sam_drag_start, pos).normalized()
                self.update()
                return
            if self.sam_mode == "point" and self.sam_client:
                # 悬停已有形状上 → 正常光标，走下面形状检测逻辑
                on_shape = any(
                    shape.contains_point(pos)
                    for shape in self.shapes if self.isVisible(shape)
                )
                if not on_shape:
                    # 悬停空白处 → 十字绘制光标
                    self.override_cursor(CURSOR_DRAW)
                    if not (ev.buttons() & Qt.LeftButton):  # 不阻断正在拖拽的操作
                        return
        ...
        if Qt.LeftButton & ev.buttons():
            if self.selected_vertex():
             ...
             # pan
             raw_pos = ev.pos()
             delta_x = raw_pos.x() - self.pan_initial_pos.x()
             delta_y = raw_pos.y() - self.pan_initial_pos.y()
             self.scrollRequest.emit(-delta_x, Qt.Horizontal)
             self.scrollRequest.emit(-delta_y, Qt.Vertical)
             self.update()
             self.pan_initial_pos = raw_pos
            return
        ...
        for shape in reversed([s for s in self.shapes if self.isVisible(s)]):
            ...
            if index is not None:
                ...
                self.update()
                break
            if shape.shape_type == "polygon" and shape == self.selected_shape:
                insert_idx, _ = shape.nearest_edge_insert_point(pos, self.epsilon)
                if insert_idx is not None:
                    if self.selected_vertex():
                        self.h_shape.highlight_clear()
                    self.h_vertex, self.h_shape = None, shape
                    self.override_cursor(CURSOR_POINT)
                    self.setToolTip("Click to add point on edge")
                    self.setStatusTip(self.toolTip())
                    self.update()
                    break
    ...
    def mousePressEvent(self, ev):
        ...
        # --- SAM 交互模式 ---
        if self.sam_enabled and self.sam_client and not self.drawing():
            if ev.button() == Qt.LeftButton:
                if self.sam_mode == "point":
                    # 没按下穿透键且点在已有形状上 → 正常编辑
                    on_shape = not self.sam_passthrough and any(
                        shape.contains_point(pos)
                        for shape in self.shapes if self.isVisible(shape)
                    )
                    # 正在 hover 顶点时不触发 SAM（允许拖拽顶点）
                    on_vertex = self.h_vertex is not None
                    if not on_shape and not on_vertex:
                        if self.sam_client.current_image is None:
                            self.samEncodeRequested.emit()
                        if self.sam_client.current_image is None:
                            return
                        self.parent().window().statusBar().showMessage("SAM点提示推理中...", 0)
                        QApplication.processEvents()
                        mask, bbox = self.sam_client.predict_point(pos.x(), pos.y())
                        if mask is not None and self.pixmap is not None:
                            area_ratio = mask.sum() / (mask.shape[0] * mask.shape[1])
                            if area_ratio > 0.9:
                                _w = self.window()
                                if _w and hasattr(_w, "_show_mode_popup"):
                                    _w._show_mode_popup("SAM\u70b9\u63d0\u793a: \u672a\u8bc6\u522b\u5230\u7279\u5b9a\u76ee\u6807")
                            else:
                                self.samResultReady.emit(
                                    {"type": "point", "mask": mask, "bbox": bbox, "prompt_label": True})
                        else:
                            _w = self.window()
                            if _w and hasattr(_w, "_show_mode_popup"):
                                _w._show_mode_popup("SAM\u70b9\u63d0\u793a: \u672a\u8bc6\u522b\u5230\u76ee\u6807")
                        return
                elif self.sam_mode == "box":
                    # 框提示：点击在已有形状上 → 正常编辑（选中/移动）
                    # 点击空白处 → 开始 SAM 拖拽框选
                    on_shape = any(
                        shape.contains_point(pos)
                        for shape in self.shapes if self.isVisible(shape)
                    ...
                    if not on_shape:
                        self.sam_drag_start = pos
                        self.sam_drag_rect = None
                        return
                    # 点在形状上：fall through 到正常逻辑
            else:
                return  # SAM 模式下忽略其他按键

        if ev.button() == Qt.LeftButton:
            if self.drawing():
                self.handle_drawing(pos)
            else:
                # Vertex detection takes priority over edge insertion
                if self.selected_shape and self.selected_shape.shape_type == "polygon":
                    if self.selected_shape.nearest_vertex(pos, self.epsilon) is None:
                        insert_idx, edge_pt = self.selected_shape.nearest_edge_insert_point(pos, self.epsilon)
                        if insert_idx is not None:
                            self.selected_shape.add_point_at(insert_idx, edge_pt)
                            self.shapeMoved.emit()
                            self.update()
                            return
    ...
    def mouseReleaseEvent(self, ev):
        # --- SAM 框提示拖拽完成 ---
        if self.sam_enabled and self.sam_mode == "box" and self.sam_drag_start and ev.button() == Qt.LeftButton:
            end_pos = self.transform_pos(ev.pos())
            dx = abs(end_pos.x() - self.sam_drag_start.x())
            dy = abs(end_pos.y() - self.sam_drag_start.y())
            if dx > 5 or dy > 5:
                x1, y1 = int(self.sam_drag_start.x()), int(self.sam_drag_start.y())
                x2, y2 = int(end_pos.x()), int(end_pos.y())
                if self.sam_client.current_image is None:
                    self.samEncodeRequested.emit()
                if self.sam_client.current_image is not None:
                    self.parent().window().statusBar().showMessage("SAM框提示推理中...", 0)
                    QApplication.processEvents()
                    mask = self.sam_client.predict_box(x1, y1, x2, y2)
                    if mask is not None:
                        bbox = self.sam_client._mask_to_bbox(mask)
                        self.samResultReady.emit({"type": "box", "mask": mask, "bbox": bbox, "prompt_label": True})
            self.sam_drag_start = None
            self.sam_drag_rect = None
            self.update()
            return
        ...
        if ev.button() == Qt.RightButton:
            ...
            pos = self.transform_pos(ev.pos())
            if self.drawing() and not self.draw_by_two_clicks:
                # For polygon mode, press already added the vertex; skip release.
    ...
    def handle_drawing(self, pos):
        if self.draw_by_two_clicks:
            if self.current is None:
                ...
                init_pos = self.current[0]
                min_x = min(init_pos.x(), pos.x())
                min_y = min(init_pos.y(), pos.y())
                max_x = max(init_pos.x(), pos.x())
                max_y = max(init_pos.y(), pos.y())
        ...
        if self.drawing_mode == 'polygon':
            ...
            if self.current is None:
                ...
                # Allow closing polygon by clicking near the start point
                if len(self.current) > 2 and self.close_enough(pos, self.current[0]):
                    self.finalise()
                else:
                    self.current.add_point(pos)
    ...
    def mouseDoubleClickEvent(self, ev):
        ...
        if self.editing() and self.selected_shape:
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            # SAM 框提示拖拽预览
            if self.sam_enabled and self.sam_drag_rect:
                p.setPen(QPen(QColor(255, 255, 0), 1, Qt.DashLine))  # 黄色虚线
                brush = QBrush(QColor(255, 255, 0, 40))
                p.setBrush(brush)
                p.drawRect(self.sam_drag_rect)

            self.setAutoFillBackground(True)
            pal = self.palette()
            pal.setColor(
                self.backgroundRole(),
                QColor(184, 239, 38, 128) if self.verified else QColor(232, 232, 232, 255),
            )
            self.setPalette(pal)
        finally:
            self._painting = False
    ...
    def keyPressEvent(self, ev):
        key = ev.key()
        if key == Qt.Key_Z and ev.modifiers() & Qt.ControlModifier and self.current:
            if len(self.current) > 1:
                self.current.pop_point()
                if len(self.current) >= 2:
                    self.line.points = [self.current[-1], self.current[-2]]
                else:
            ...
            if self.current:
                # Stage 1: cancel current polygon being drawn
                self.current = None
                self.line.points = []
                self.drawingPolygon.emit(False)
                self.set_hiding(False)
                ...
                # Stage 2: completely exit drawing mode
    ...
    def keyReleaseEvent(self, ev):
        if ev.key() == Qt.Key_Space and self.sam_enabled and self.sam_passthrough:
            self.sam_passthrough = False
            self.unsetCursor()
            self._cursor = CURSOR_DEFAULT
            ev.accept()
    ...
    def update_sam_cursor(self, color):
        pixmap = QPixmap(32, 32)
        pixmap.fill(Qt.transparent)
        painter = QPainter(pixmap)
        painter.setRenderHint(QPainter.Antialiasing)
        painter.fillRect(pixmap.rect(), QColor(255, 255, 255, 32))
        pen = QPen(color, 2)
        painter.setPen(pen)
        h = 16
        painter.drawLine(0, h, h - 3, h)
        painter.drawLine(h + 3, h, 32, h)
        painter.drawLine(h, 0, h, h - 3)
        painter.drawLine(h, h + 3, h, 32)
        painter.drawPoint(h, h)
        painter.end()
        self._sam_cross_cursor = QCursor(pixmap, h, h)
    ...
    def set_drawing_by_two_clicks(self, status):
        self.draw_by_two_clicks = status
